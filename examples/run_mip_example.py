...
if __name__ == '__main__':
    ...
    print("✨ Interpolating 3D volumes (this may take a moment)...", flush=True)
    interpolated_volume = interpolator_3d(volume1, volume2, dt)
    print(f"✅ Interpolation complete. Output shape: {interpolated_volume.shape}", flush=True)

    print("📊 Performing Maximum Intensity Projections...", flush=True)
    ...
    # Interpolated result is RGB, so we can display it directly
    # Clip values to [0, 1] to avoid matplotlib warnings if model outputs slightly out of range
    ...
    print(f"🖼️  Comparison image saved to: {out_path}", flush=True)
