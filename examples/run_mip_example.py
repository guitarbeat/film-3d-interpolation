...
if __name__ == '__main__':
    ...
    # Plot Start (Volume 1)
    # Using the first channel (index 0) assuming grayscale or similar structure
    im1 = axes[0].imshow(mip_v1[0, :, :, 0], cmap='gray', vmin=0, vmax=1)
    axes[0].set_title("Start (t=0)")
    axes[0].axis('off')

    # Plot Interpolated
    im2 = axes[1].imshow(mip_interp[0, :, :, 0], cmap='gray', vmin=0, vmax=1)
    axes[1].set_title("Interpolated (t=0.5)")
    axes[1].axis('off')

    # Plot End (Volume 2)
    im3 = axes[2].imshow(mip_v2[0, :, :, 0], cmap='gray', vmin=0, vmax=1)
    ...
    fig.colorbar(im2, cax=cbar_ax, label='Intensity')
