...
# Suppress TF logs and warnings for cleaner CLI output
...
if __name__ == '__main__':
    print("🚀 Starting 3D Volume Interpolation Example...", flush=True)
    ...
    print(f"✅ Interpolation complete. Interpolated volume shape: {interpolated_volume.shape}", flush=True)

    print("🖼️  Performing Maximum Intensity Projection...", flush=True)
    mip_v1 = max_intensity_projection(volume1, axis=1)
    mip_interpolated = max_intensity_projection(interpolated_volume, axis=1)
    mip_v2 = max_intensity_projection(volume2, axis=1)
    print(f"✅ MIP images generated.", flush=True)

    out_dir = os.path.join(os.path.dirname(__file__), 'outputs')
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, 'interpolated_mip.png')

    print("💾 Saving side-by-side comparison plot...", flush=True)
    # Side-by-side plot
    ...
    # Interpolated
    # Note: interpolated volume is RGB, so we don't use cmap='gray' and we clip it just in case.
    axes[1].imshow(np.clip(mip_interpolated[0], 0, 1))
    ...
    print(f"✨ Success! Comparison image saved to {out_path}", flush=True)
