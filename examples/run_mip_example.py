...
# Suppress verbose TensorFlow logs and warnings for a cleaner CLI experience
...
if __name__ == '__main__':
    print("🤖 Initializing 3D Interpolator...", end='', flush=True)
    interpolator_3d = Interpolator3D()
    print(" Done!")

    print("🧪 Creating dummy 3D data...", end='', flush=True)
    ...
    print("✨ Interpolating 3D volumes...", end='', flush=True)
    interpolated_volume = interpolator_3d(volume1, volume2, dt)
    print(" Done!")
    print(f"   Shape: {interpolated_volume.shape}")

    print("📊 Performing Maximum Intensity Projection...", end='', flush=True)
    # Calculate MIP for start, end, and interpolated volumes for comparison
    ...
    plt.suptitle("3D Volume Interpolation Results (MIP View)", fontsize=16)
    plt.tight_layout()
