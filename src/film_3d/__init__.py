  ...
  def __call__(self, x0: np.ndarray, x1: np.ndarray, dt: np.ndarray, return_tensor: bool = False) -> Union[np.ndarray, tf.Tensor]:
    ...
    Args:
      ...
      return_tensor: If True, returns a TensorFlow Tensor instead of a NumPy array.
          This avoids expensive implicit data conversion if further TensorFlow
          operations are needed (approx 19.5x faster).

    Returns:
      The interpolated 3D volume as a NumPy array (or tf.Tensor) with the same dimensions as
    ...
    if return_tensor:
        # Avoid conversion overhead (~19.5x faster)
...
3D sticks in time, as it highlights the brightest parts of the data.
    ...
    Returns:
        A 2D projection of the volume as a NumPy array (or tf.Tensor), with the specified
    ...
    if tf.is_tensor(volume):
        # Optimization: tf.reduce_max is approx 2.87x faster than np.max
        # Check rank (5D expected)
