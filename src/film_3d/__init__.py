...
class Interpolator3D:
  ...
  @tf.function
  def _run_inference(self, x0: tf.Tensor, x1: tf.Tensor, time: tf.Tensor) -> tf.Tensor:
    """Runs inference using the FILM model within a tf.function graph.

    This encapsulates the model call and channel expansion to minimize
    Python overhead and enable graph optimizations.
    """
  ...
  def __call__(self, x0: np.ndarray, x1: np.ndarray, dt: np.ndarray) -> np.ndarray:
    ...
    # Perform inference using the optimized _run_inference method.
    # Note: slice0_padded is already a Tensor (from _pad_to_align), and dt_reshaped
    # will be automatically converted to Tensor by tf.function.
